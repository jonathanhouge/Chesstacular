...
public class Chessboard implements ChessBoardUI {
	...
	private Piece makePiece(int x, int y, boolean playerColor) {
		...
		if (y == 0) { // enemy specialty pieces
			if (x == 0 || x == 7) {
				return (new Rook(playerColor, shell));
			} else if (x == 0 || x == 7) {
				return (new Rook(playerColor, shell));
			} else if (x == 1 || x == 6) {
				return (new Knight(playerColor, shell));
			} else if (x == 2 || x == 5) {
				return (new Bishop(playerColor, shell));
			} else if (x == 3) {
				return (new King(playerColor, shell));
			} else if (x == 4) {
				return (new Queen(playerColor, shell));
			}
		} else if (y == 1) {
			return (new Pawn(playerColor, shell));
		} // enemy pawns
